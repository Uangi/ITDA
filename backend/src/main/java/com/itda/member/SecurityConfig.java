...
import com.itda.member.UserSecurityService;
...
@Configuration
@EnableWebSecurity // Spring Security를 활성화하는 데 사용됩니다. 이를 통해 Spring Security의 설정을 사용
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/**").permitAll()
                .and()
                .csrf().ignoringAntMatchers("/oracle-console/**") // CSRF(Cross-Site Request Forgery) 보호를 구성합니다. 여기서는 H2
                // 데이터베이스 콘솔과 같은 특정 URL 패턴을 CSRF 보호에서 제외
                .and()
                .headers()
                .addHeaderWriter(new XFrameOptionsHeaderWriter(XFrameOptionsHeaderWriter.XFrameOptionsMode.SAMEORIGIN))
                .and() // HTTP 헤더에 X-Frame-Options를 추가하여 클릭재킹 공격을 방지합니다. SAMEORIGIN 모드는 해당 페이지를 포함한
                       // 프레임을 동일한 출처에서만 허용
                .formLogin().loginPage("/user/login").defaultSuccessUrl("/")
                .and() // 사용자의 로그인 페이지 및 로그인 후 기본적으로 리다이렉트될 URL을 설정합니다.
                .logout().logoutRequestMatcher(new AntPathRequestMatcher("/user/logout")) // /user/logout 경로로 요청이 올 때
                                                                                          // 로그아웃을 처리
                .logoutSuccessUrl("/").invalidateHttpSession(true) // 세션 무효화 후 루트 url(index) 로 리다이렉트
